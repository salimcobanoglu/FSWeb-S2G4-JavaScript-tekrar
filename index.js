...
function CemberinCevresi(cemberinYariCapiP){

const cevresi = (2 * pi * cemberinYariCapiP);
return cevresi
...
function CemberinAlani(cemberinYariCapiP,piP){

const cemberinAlani = piP * Math.pow(cemberinYariCapiP,2);
...
/* 	GÖREV 3:
		- Sayfanın en üstünde global değişken olarak tanımlanmış bir sayilar dizisi bulunmaktadır. Bu dizi içinde 0 ile 1000 arasında rasgele 
		oluşturulmuş tam sayılar ve ondalıklı sayılar bulunmaktadır. Bu diziyi kullanarak aşağıdakileri uygulayın:
			...
			için en az 2 kere döngü yazmalısınız. Birinci döngüde hangi sayının kaç kere tekrar edildiğini tespit edip, 2. döngüde stringi 
			oluşturup verilen diziye aktarmalısınız.
	*/
...
console.log(sayilar.length)
	...
	//3a çözümü - 3a. enbuyuk ve enkucuk isminde 2 adet değişken tanımlayın ve sayilar dizisindeki en küçük sayı ile en büyük sayıyı bu değişkenlere 
	//atayın.(for döngüsü kullanın)
...
console.log("En Büyük Sayı: ", enkucuk)
	...
	var ucebolunenlerintoplami = ucetambolunenler.reduce((onceki,mevcut) => {return onceki + mevcut},0)
	...
	var besyuzdenkucuksayilar = sayilar.filter((x) => x < 500);
...
console.log(siralisayilar); 
	...
	*/
	...
	const tekrarEdenSayilar = tekraredenlerBurada(sayilar);

	function tekraredenlerBurada(sayilarP){
	const tekraredenler = []
	 for (let i=0; i<sayilarP.length; i++) {
	   if (sayilarP[i] == sayilarP[i+1]) {  
		 tekraredenler.push(sayilarP[i])
	   }
	 }
	  return tekraredenler;
	}
	console.log(tekraredenlerBurada(sayilar))
